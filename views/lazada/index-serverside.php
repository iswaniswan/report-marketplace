...
?>
<form action="<?= Url::to(['lazada/index-serverside']) ?>" method="GET">
    <div class="row mb-4">
               ...
               <table class="table table-hover table-bordered" id="table-serverside">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 8%;">Order Item ID</th>
                            <th style="width: 24%;">Item Name</th>
                            <th style="width: 8%;">Seller SKU</th>
                            <th style="width: 8%;">Variation</th>
                            <th style="width: 8%;">Status</th>
                            <th style="width: 8%;">Order Number</th>
                            <th style="width: 8%;">Unit Price</th>
                            <th style="width: 8%;">Seller Discount Total</th>
                            <th>Action</th>
                        </tr>
                    </thead>
               </table>
...
<?php
$urlServerside = Url::to(['lazada/serverside']);
$urlExportAll = Url::to(['lazada/export-all']);
...
$countAllStatus = count(Lazada::getListStatus());
$isAutoCheckedAll = (@$status == '' || empty(@$status) || @$status[0] == '') ? 1 : 0;

$headerPrint = 'Shopee ' . date('d M Y', strtotime($date_start)) . ' - ' . date('d M Y', strtotime($date_end));
...
$script = <<<JS
    ...
    $('#table-serverside').DataTable({
        ...
        columns: [
            ...
            { data: 'order_item_id', orderable: false},
            { data: 'item_name', orderable: false},
            { data: 'seller_sku', orderable: false},
            { data: 'variation', orderable: false},
            { data: 'status', orderable: false},
            { data: 'order_number', orderable: false},
            { data: 'unit_price', orderable: false},
            { data: 'seller_discount_total', orderable: false},
